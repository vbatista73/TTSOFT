function [v1,v2,vc]=monV(xVec,d)
% Calculates v1, v2, vc in TT format for monomers
% Using parameters from Raab et al. (1999) doi:10.1063/1.478061
...
%------------------------- v1 v2-------------------------
v1=tt_zeros(nx,d)-delta;
v2=tt_zeros(nx,d)+delta;

dimsB4=0;
for a=1:min(ag,d-dimsB4)
    pot1=tt_tensor(wag(a)*xSq/2+aiag(a)*xVec+aijag(a,a)*xSq);
    pot2=tt_tensor(wag(a)*xSq/2+biag(a)*xVec+bijag(a,a)*xSq);
    if a-1>0
        pot1=tkron(tt_ones(nx,a-1+dimsB4),pot1);
        pot2=tkron(tt_ones(nx,a-1+dimsB4),pot2);
    end
    if d-a-dimsB4>0
        pot1=tkron(pot1,tt_ones(nx,d-a-dimsB4));
        pot2=tkron(pot2,tt_ones(nx,d-a-dimsB4));
    end
    v1=v1+pot1;
    v2=v2+pot2;
end
...
for a=1:min(b1g,d-dimsB4)
    pot1=tt_tensor(wb1g(a)*xSq/2+aijb1g(a,a)*xSq);
    pot2=tt_tensor(wb1g(a)*xSq/2+bijb1g(a,a)*xSq);
    pot1=tkron(tt_ones(nx,a-1+dimsB4),pot1);
    pot2=tkron(tt_ones(nx,a-1+dimsB4),pot2);
    if d-a-dimsB4>0
        pot1=tkron(pot1,tt_ones(nx,d-a-dimsB4));
...
for a=1:min(b2g,d-dimsB4)
    pot1=tt_tensor(wb2g(a)*xSq/2+aijb2g(a,a)*xSq);
    pot2=tt_tensor(wb2g(a)*xSq/2+bijb2g(a,a)*xSq);
    pot1=tkron(tt_ones(nx,a-1+dimsB4),pot1);
    pot2=tkron(tt_ones(nx,a-1+dimsB4),pot2);
    if d-a-dimsB4>0
        pot1=tkron(pot1,tt_ones(nx,d-a-dimsB4));
...
for a=1:min(b3g,d-dimsB4)
    pot1=tt_tensor(wb3g(a)*xSq/2+aijb3g(a,a)*xSq);
    pot2=tt_tensor(wb3g(a)*xSq/2+bijb3g(a,a)*xSq);
    pot1=tkron(tt_ones(nx,a-1+dimsB4),pot1);
    pot2=tkron(tt_ones(nx,a-1+dimsB4),pot2);
    if d-a-dimsB4>0
        pot1=tkron(pot1,tt_ones(nx,d-a-dimsB4));
...
for a=1:min(au,d-dimsB4)
    pot1=tt_tensor(wau(a)*xSq/2+aijau(a,a)*xSq);
    pot2=tt_tensor(wau(a)*xSq/2+bijau(a,a)*xSq);
    pot1=tkron(tt_ones(nx,a-1+dimsB4),pot1);
    pot2=tkron(tt_ones(nx,a-1+dimsB4),pot2);
    if d-a-dimsB4>0
        pot1=tkron(pot1,tt_ones(nx,d-a-dimsB4));
...
for a=1:min(b1u,d-dimsB4)
    pot1=tt_tensor(wb1u(a)*xSq/2+aijb1u(a,a)*xSq);
    pot2=tt_tensor(wb1u(a)*xSq/2+bijb1u(a,a)*xSq);
    pot1=tkron(tt_ones(nx,a-1+dimsB4),pot1);
    pot2=tkron(tt_ones(nx,a-1+dimsB4),pot2);
    if d-a-dimsB4>0
        pot1=tkron(pot1,tt_ones(nx,d-a-dimsB4));
...
for a=1:min(b2u,d-dimsB4)
    pot1=tt_tensor(wb2u(a)*xSq/2+aijb2u(a,a)*xSq);
    pot2=tt_tensor(wb2u(a)*xSq/2+bijb2u(a,a)*xSq);
    pot1=tkron(tt_ones(nx,a-1+dimsB4),pot1);
    pot2=tkron(tt_ones(nx,a-1+dimsB4),pot2);
    if d-a-dimsB4>0
        pot1=tkron(pot1,tt_ones(nx,d-a-dimsB4));
...
for a=1:min(b3u,d-dimsB4)
    pot1=tt_tensor(wb3u(a)*xSq/2+aijb3u(a,a)*xSq);
    pot2=tt_tensor(wb3u(a)*xSq/2+bijb3u(a,a)*xSq);
    pot1=tkron(tt_ones(nx,a-1+dimsB4),pot1);
    pot2=tkron(tt_ones(nx,a-1+dimsB4),pot2);
    if d-a-dimsB4>0
        pot1=tkron(pot1,tt_ones(nx,d-a-dimsB4));
...
%----------------------- vc --------------------------------------
if d>ag
    vc=tt_tensor(cib1g(1)*xVec*3.6749d-2);
    vc=tkron(tt_ones(nx,ag),vc);
    if d-1-ag>0
        vc=tkron(vc,tt_ones(nx,d-1-ag));
    end
else
    vc=tt_zeros(nx,d);
end
